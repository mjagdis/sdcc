...
cl_z80::make_memories(void)
{
  ...
  vars->add(v= new cl_var(cchars("ALT_A"), regs8, 8, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_F"), regs8, 9, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_B"), regs8, 10, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_C"), regs8, 11, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_D"), regs8, 12, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_E"), regs8, 13, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_H"), regs8, 14, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_L"), regs8, 15, "", 7, 0));
  v->init();
