...
cl_r2k::make_memories(void)
{
  ...
  v->init();
  vars->add(v= new cl_var(cchars("F"), regs8, 1, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("B"), regs8, 2, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("C"), regs8, 3, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("D"), regs8, 4, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("E"), regs8, 5, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("H"), regs8, 6, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("L"), regs8, 7, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("IIR"), regs8, 8, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("EIR"), regs8, 9, "", 7, 0));
  v->init();

  vars->add(v= new cl_var(cchars("ALT_A"), regs8, 10, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_F"), regs8, 11, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_B"), regs8, 12, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_C"), regs8, 13, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_D"), regs8, 14, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_E"), regs8, 15, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_H"), regs8, 16, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_L"), regs8, 17, "", 7, 0));
  v->init();

  vars->add(v= new cl_var(cchars("AF"), regs16, 0, "", 15, 0));
  v->init();
  vars->add(v= new cl_var(cchars("BC"), regs16, 1, "", 15, 0));
  v->init();
  vars->add(v= new cl_var(cchars("DE"), regs16, 2, "", 15, 0));
  v->init();
  vars->add(v= new cl_var(cchars("HL"), regs16, 3, "", 15, 0));
  v->init();
  vars->add(v= new cl_var(cchars("IX"), regs16, 4, "", 15, 0));
  v->init();
  vars->add(v= new cl_var(cchars("IY"), regs16, 5, "", 15, 0));
  v->init();
  vars->add(v= new cl_var(cchars("SP"), regs16, 6, "", 15, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_AF"), regs16, 7, "", 15, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_BC"), regs16, 8, "", 15, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_DE"), regs16, 9, "", 15, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_HL"), regs16, 10, "", 15, 0));
  v->init();
}
