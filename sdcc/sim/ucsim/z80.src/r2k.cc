...
cl_r2k::make_memories(void)
{
  ...
  regs8= new cl_address_space("regs8", 0, sizeof(regs)+2, 8);
  regs8->init();
  regs8->decode(0, (new cl_chip_8bit("regs8", sizeof(regs), (u8_t *)&regs))->chip_init());
  regs8->decode(9, (new cl_chip_8bit("iir", 1, (u8_t *)&iir))->chip_init());
  regs8->decode(10, (new cl_chip_8bit("eir", 1, (u8_t *)&eir))->chip_init());

  regs16= new cl_address_space("regs16", 0, sizeof(regs)/2, 16);
  regs16->init();
  regs16->decode(0, (new cl_chip_16bit("regs16", sizeof(regs)/2, (u16_t *)&regs))->chip_init());

  address_spaces->add(regs8);
  address_spaces->add(regs16);

  class cl_var *v;
  vars->add(v= new cl_var(cchars("A"), regs8, (u8_t *)&regs.raf.A - (u8_t *)&regs.raf.A, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("F"), regs8, (u8_t *)&regs.raf.A - (u8_t *)&regs.raf.A, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("B"), regs8, (u8_t *)&regs.bc.h - (u8_t *)&regs.raf.A, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("C"), regs8, (u8_t *)&regs.bc.l - (u8_t *)&regs.raf.A, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("D"), regs8, (u8_t *)&regs.de.h - (u8_t *)&regs.raf.A, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("E"), regs8, (u8_t *)&regs.de.l - (u8_t *)&regs.raf.A, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("H"), regs8, (u8_t *)&regs.hl.h - (u8_t *)&regs.raf.A, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("L"), regs8, (u8_t *)&regs.hl.l - (u8_t *)&regs.raf.A, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("IIR"), regs8, 9, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("EIR"), regs8, 10, "", 7, 0));
  v->init();

  vars->add(v= new cl_var(cchars("ALT_A"), regs8, (u8_t *)&regs.ralt_af.aA - (u8_t *)&regs.raf.A, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_F"), regs8, (u8_t *)&regs.ralt_af.aF - (u8_t *)&regs.raf.A, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_B"), regs8, (u8_t *)&regs.a_bc.h - (u8_t *)&regs.raf.A, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_C"), regs8, (u8_t *)&regs.a_bc.l - (u8_t *)&regs.raf.A, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_D"), regs8, (u8_t *)&regs.a_de.h - (u8_t *)&regs.raf.A, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_E"), regs8, (u8_t *)&regs.a_de.l - (u8_t *)&regs.raf.A, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_H"), regs8, (u8_t *)&regs.a_hl.h - (u8_t *)&regs.raf.A, "", 7, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_L"), regs8, (u8_t *)&regs.a_hl.l - (u8_t *)&regs.raf.A, "", 7, 0));
  v->init();

  vars->add(v= new cl_var(cchars("AF"), regs16, (u16_t *)&regs.AF - (u16_t *)&regs.AF, "", 15, 0));
  v->init();
  vars->add(v= new cl_var(cchars("BC"), regs16, (u16_t *)&regs.BC - (u16_t *)&regs.AF, "", 15, 0));
  v->init();
  vars->add(v= new cl_var(cchars("DE"), regs16, (u16_t *)&regs.DE - (u16_t *)&regs.AF, "", 15, 0));
  v->init();
  vars->add(v= new cl_var(cchars("HL"), regs16, (u16_t *)&regs.HL - (u16_t *)&regs.AF, "", 15, 0));
  v->init();
  vars->add(v= new cl_var(cchars("IX"), regs16, (u16_t *)&regs.IX - (u16_t *)&regs.AF, "", 15, 0));
  v->init();
  vars->add(v= new cl_var(cchars("IY"), regs16, (u16_t *)&regs.IY - (u16_t *)&regs.AF, "", 15, 0));
  v->init();
  vars->add(v= new cl_var(cchars("SP"), regs16, (u16_t *)&regs.SP - (u16_t *)&regs.AF, "", 15, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_AF"), regs16, (u16_t *)&regs.aAF - (u16_t *)&regs.AF, "", 15, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_BC"), regs16, (u16_t *)&regs.aBC - (u16_t *)&regs.AF, "", 15, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_DE"), regs16, (u16_t *)&regs.aDE - (u16_t *)&regs.AF, "", 15, 0));
  v->init();
  vars->add(v= new cl_var(cchars("ALT_HL"), regs16, (u16_t *)&regs.aHL - (u16_t *)&regs.AF, "", 15, 0));
  v->init();
}
...
void
cl_r2k::disass(class cl_console_base *con, t_addr addr, const char *sep)
{
  const char *b;
  int len = 0;
  int immed_offset = 0;
