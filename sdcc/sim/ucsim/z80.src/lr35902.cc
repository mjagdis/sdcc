...
cl_lr35902::make_memories(void)
{
  ...
  regs8= new cl_address_space("regs8", 0, sizeof(regs), 8);
  regs8->init();
  ...
  v->init();
  vars->add(v= new cl_var(cchars("F"), regs8, (u8_t *)&regs.raf.F - (u8_t *)&regs.raf.A, "", 7, 0));
  v->init();
  ...
  v->init();
  vars->add(v= new cl_var(cchars("L"), regs8, (u8_t *)&regs.hl.h - (u8_t *)&regs.raf.A, "", 7, 0));
  v->init();
...
void
cl_lr35902::disass(class cl_console_base *con, t_addr addr, const char *sep)
{
  const char *b;
  int len = 0;
  int immed_offset = 0;
